...
public class DetailBoardController extends HttpServlet {
	...
	public DetailBoardController() {
		...
		this.commentdao = MyAppSqlConfig.getSqlSessionInstance().getMapper(CommentDAO.class);
	}
...
//		int no = getInt("조회할 글번호를 입력하세요 : ");
		...
		req.setAttribute("comment", commentdao.selectComment(Integer.parseInt(req.getParameter("no"))));
		
		int commentNo = req.getParameter("commentNo") == null ? 0 : Integer.parseInt(req.getParameter("commentNo"));
