	...
	<div class="content">
		...
		<c:if test="${pr.count ne 0}">
			<nav>
			  ...
			  <c:if test="${pr.prev}">
			    <li>
			      <a href="list3.do?pageNo=${pr.beginPage - 1}" aria-label="Previous">
			        <span aria-hidden="true">&laquo;</span>
			      </a>
			    </li>
			  </c:if>  
			    <c:forEach var="i" begin="${pr.beginPage}" end="${pr.endPage}">			   
			    <li <c:if test="${pr.pageNo eq i}">class="active"</c:if>><a href="list3.do?pageNo=${i}">${i}</a></li>			    
			    </c:forEach>
			    <c:if test="${pr.next}">
			    <li>
			      <a href="list3.do?pageNo=${pr.endPage + 1}" aria-label="Next">
