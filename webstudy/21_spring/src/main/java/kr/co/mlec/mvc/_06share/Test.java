...
public class Test {
	...
	public void test02(Member member) {	// 알아서 공유영역에 올라가고 클래스 첫자를 소문자로 바꿔 공유한다.
		System.out.println("test02 메서드 호출됨..");
	}
	
	@RequestMapping("/test03.do")
	public void test03(@ModelAttribute("m") Member member) {	// ModelAttribute를 주면 파라미터로 넘어온 값이 m이란 이름으로 공유영역에 올라간다.
	...
	public String test06(RedirectAttributes attr) {
		...
		// redirect 할 때 한번만 파라미터를 넘길 수 있게 제공해줌
