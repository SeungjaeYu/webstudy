...
function queryResult(query, queryType, res, info, queryStr) {
	...
	switch (queryType) {
		case "selectList" :
			...
			break;
		case "selectOne" :
			con.query(query, queryStr, (err, {title, writer, content}) => {
				if (err) errDBProcess(err);
				resultSet = {title, writer, content};
				send(res,info,resultSet);
			});
...
http.createServer((req, res) => {
   ...
   let param;
   if ()
   let param = qs.parse(req.url.substr(req.url.indexOf("?") + 1,req.url.length));
   console.log("req:" + req.url.substr(req.url.indexOf("?") + 1,req.url.length));
   //if ()
   console.log(param);
   ...
   if (req.method === "GET") {
	  ...
	  if (info === "view/board/list.html") {
		  ...
		  let param = qs.parse(req.url);
		  let no = param.no;
		  queryResult(query, "selectOne",res, info, no);
