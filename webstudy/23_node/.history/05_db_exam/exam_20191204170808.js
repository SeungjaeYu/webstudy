...
let mappings = [];
function setMapping(url, fileName){
	mappings[url]
}
...
http.createServer((req, res) => {
   if (req.url === "/favicon.ico") {
	...
	else if (req.url.indexOf("?") != -1 && req.url.indexOf("/updateform.do") != -1) {
		param = qs.parse(req.url.substr(req.url.indexOf("?") + 1,req.url.length));
		setMapping("/updateform.do" + req.url.substr(req.url.indexOf("?"),req.url.length), "view/board/updateForm.html");
	}
   ...
   if (req.method === "GET") {
	  ...
	  if (info === "view/board/list.html") {
		...
		} else if (info === "view/board/detail.html" || info === "view/board/updateForm.html") {
      ...
      req.on("end", () => {
		  ...
		  if (req.url === "/write.do") {
			...
			`;
			let param = qs.parse(paramData);
			queryResult(query, "update", res, info, [param.title, param.content, param.no]);
...
function send(response, fileName, resultValue, redirectURL) {
   ...
   fs.readFile(fileName, (err, data) => {
	  ...
	  } else if (fileName == "view/board/detail.html" || fileName == "view/board/updateForm.html") {
