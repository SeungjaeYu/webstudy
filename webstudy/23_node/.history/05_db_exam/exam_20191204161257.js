	...
	switch (queryType) {
		...
		case "selectOne" :
			...
			con.query(query, queryStr, (err, result) => {
				...
				if (err) errDBProcess(err);
				result.forEach(({title, writer, content}) => {
...
http.createServer((req, res) => {
   if (req.url === "/favicon.ico") {
	...
	if (req.url.indexOf("?") != -1) {
		...
		console.log(req.url.substr(req.url.indexOf(0), req.url.indexOf("?")));
		setMapping(req.url.substr(req.url.indexOf(0), req.url.indexOf("?")) + req.url.substr(req.url.indexOf("?"),req.url.length), "view/board/detail.html");
	}
